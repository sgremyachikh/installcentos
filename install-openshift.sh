...
export PVS=${PVS:="true"}
export DOMAIN=${DOMAIN:="my.wildcard.domain"}

export USERNAME=${USERNAME:="openshift_admin_login"}
export PASSWORD=${PASSWORD:="openshift_admin_password"}
export ADMIN_USERNAME=${ADMIN_USERNAME:="marfa"}
export ADMIN_PASSWORD=${ADMIN_PASSWORD:="Fie0za6n"}
...
if [ "$INTERACTIVE" = "true" ]; then
	...
	read -rp "OpenShift admin Username: ($USERNAME): " choice;
	if [ "$choice" != "" ] ; then
		export USERNAME="$choice";
	fi

	read -rp "OpenShift admin Password: ($PASSWORD): " choice;
	if [ "$choice" != "" ] ; then
...
# replacing sync.yml with "--insecure" flag to success curl of api server with self-signet certs
...
# Let's Encrypt created certs deploy setup
